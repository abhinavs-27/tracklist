...
export function CommunityLeaderboardClient({
  ...
  const [prevServerPrimed, setPrevServerPrimed] = useState(serverPrimed);
  if (serverPrimed !== prevServerPrimed) {
    setPrevServerPrimed(serverPrimed);
    if (serverPrimed) setLoading(false);
  }

  useEffect(() => {
    if (serverPrimed) return;
    void (async () => {
      setLoading(true);
      setError(null);
