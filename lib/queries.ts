...
/** One round-trip: migration 140 `get_home_feed_legacy_bundle`. */
async function tryHomeFeedLegacyBundle(
  supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>,
  userId: string,
  fetchLimit: number,
  ...
  const { data, error } = await supabase.rpc("get_home_feed_legacy_bundle", {
  ...
  if (error) {
    console.warn(
      "[queries] get_home_feed_legacy_bundle RPC unavailable, using parallel RPCs:",
      error.message,
    );
    return null;
  }
  ...
  const listenSessions = Array.isArray(listensRaw)
    ? listensRaw.map(parseListenSessionFromBundleJson).filter((x): x is FeedListenSessionRow => x != null)
    : [];
  return { reviewRows, followRows, listenSessions };
}

async function buildActivityFeedFromLegacyRows(
  supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>,
  cappedLimit: number,
  reviewRows: FeedReviewRpcRow[],
  followRows: FeedFollowRpcRow[],
