...
export async function fetchUserSummaryMap(
  ...
  const supabaseClient = supabase ?? (await createSupabaseServerClient());
  return fetchUserMap(supabaseClient, userIds, "id, username, avatar_url");
}
...
export async function getListenLogsForUser(
  ...
  return getListenLogsInternal({ userId, limit, offset, spotifyTrackId, supabase });
}
...
export async function getFollowCounts(
  ...
  try {
    const supabaseClient = supabase ?? (await createSupabaseServerClient());
    const { data, error } = await supabaseClient.rpc("get_follow_counts", {
      p_user_id: userId,
    });
    if (!error && data != null) {
