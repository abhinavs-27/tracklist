...
describe('Critical Flows: Automated API Logic (Vitest)', () => {
  ...
  describe('POST /api/reviews', () => {
    it('should successfully create a review and truncate long text', async () => {
      ...
      chain.single.mockResolvedValueOnce({
        data: {
          ...
          rating: 5,
          review_text: 'A'.repeat(LIMITS.REVIEW_CONTENT),
      ...
      // Verify truncation in the upsert call
    ...
    it('should return 401 for unauthorized access', async () => {
      ...
      // withHandler will catch the error from requireApiAuth and call handleUnauthorized
  ...
  describe('POST /api/logs', () => {
    ...
    it('should successfully log a listen with spotify_id', async () => {
      ...
      mockSupabase.from.mockReturnValue(chain);
      chain.single.mockResolvedValue({
        data: { id: 'l2', track_id: 'track-uuid', listened_at: new Date().toISOString() },
        error: null
      });
  ...
  describe('GET /api/search', () => {
    it('should return search results and clamp limit to 10', async () => {
