...
describe('Critical Flows: Automated API Logic (Vitest)', () => {
  ...
  describe('Creating Reviews (POST /api/reviews)', () => {
    it('should successfully create a review and truncate long text', async () => {
      ...
      const expectedTruncated = 'a'.repeat(10000);
      ...
      expect(chain.upsert).toHaveBeenCalledWith(
        expect.objectContaining({ review_text: expectedTruncated }),
        expect.anything()
      );
    });

    it('should return 401 unauthorized if auth fails', async () => {
        const req = new NextRequest('http://localhost/api/reviews', {
            method: 'POST',
        ...
        // When using withHandler, we need to handle the fact that it calls requireApiAuth internally
