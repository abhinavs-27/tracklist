...
vi.mock('@/lib/auth', () => ({
  ...
  handleUnauthorized: vi.fn(() => ({ status: 401, json: async () => ({ error: 'Unauthorized' }) })),
}));
...
vi.mock('@/lib/spotify', () => ({
  searchSpotify: vi.fn(async (q, types, limit) => {
    ...
    const results: any = { artists: { items: [] }, albums: { items: [] }, tracks: { items: [] } };
    if (types.includes('artist')) {
        results.artists.items = Array(limit || 1).fill({ id: 'a1', name: 'Test Artist' });
    }
    return results;
  }),
}));
...
vi.mock('@/lib/queries', () => ({
  ...
  fetchUserSummary: vi.fn(async (userId) => {
      if (userId === 'test-user-id') return { id: 'test-user-id', username: 'testuser', avatar_url: null };
      return null;
  }),
...
describe('Critical Flows: Automated API Logic (Vitest)', () => {
  ...
  describe('POST /api/reviews', () => {
    it('should successfully create a review', async () => {
      ...
      const res = await reviewPOST(req, { params: Promise.resolve({}), user: { id: 'test-user-id' } } as any);
    ...
    it('should truncate review text at 10k characters', async () => {
        const chain = createChain();
        ...
        chain.single.mockResolvedValueOnce({
            data: { id: 'r1', rating: 5, review_text: 'truncated', created_at: new Date().toISOString() },
            error: null
        });
        ...
        await reviewPOST(req, { params: Promise.resolve({}), user: { id: 'test-user-id' } } as any);

        expect(chain.upsert).toHaveBeenCalled();
        const upsertCall = chain.upsert.mock.calls[0][0];
        expect(upsertCall.review_text.length).toBe(10000);
    });
  });
  ...
  describe('POST /api/logs', () => {
    it('should successfully log a listen', async () => {
      ...
      const res = await logPOST(req, { params: Promise.resolve({}), user: { id: 'test-user-id' } } as any);
      expect(res.status).toBe(200);
      const body = await res.json();
  ...
  describe('GET /api/search', () => {
    it('should return search results and clamp limit', async () => {
      ...
      expect(searchSpotify).toHaveBeenCalledWith('test', expect.any(Array), 10);
    });
