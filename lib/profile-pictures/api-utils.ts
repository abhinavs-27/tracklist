...
/**
 * Handles validation, configuration check, and redirection to a presigned S3 GET URL
 * for user or community profile pictures.
 */
export async function handleProfilePictureRedirect(
  ...
  if (!id?.trim() || !isValidUuid(id)) {
    return apiNotFound(`${type === 'user' ? 'User' : 'Community'} not found`);
  }
  ...
  try {
    ...
    if (
      ...
      console.log(
        `[profile-pictures] GET /${type} → redirect presigned GetObject`,
        { id, key, presignedGet: presignedGet.slice(0, 120) + "…" },
      );
    }
