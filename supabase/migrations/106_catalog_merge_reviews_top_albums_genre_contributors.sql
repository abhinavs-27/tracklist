-- Further merge fixes: UNIQUE constraints that aborted merge_catalog_duplicate_entities.
--
-- 1) reviews: UNIQUE (user_id, entity_type, entity_id) — must drop loser rows when the user
--    already has a review for the winner *before* UPDATE entity_id.
-- 2) user_top_albums: PRIMARY KEY (user_id, album_id, period) — merge like user_top_artists.
-- 3) user_listening_genre_contributors: uq_user_genre_contrib_bucket_artist — merge play_count
--    then delete conflicting loser rows before UPDATE artist_id.
--
-- Apply after 105.
...
COMMENT ON FUNCTION public.merge_catalog_duplicate_entities(INT) IS
  'Merges duplicate catalog rows. Requires 103–106: repoint RPCs + unique-conflict fixes (reviews, favorites, spotify_recent, user_top_*, genre contributors). Run: SELECT * FROM merge_catalog_duplicate_entities();';
