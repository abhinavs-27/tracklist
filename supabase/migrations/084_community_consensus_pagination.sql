-- Paginate consensus rankings (offset + limit).

DROP FUNCTION IF EXISTS get_community_consensus_rankings(UUID, TEXT, TIMESTAMPTZ, INT);
...
COMMENT ON FUNCTION get_community_consensus_rankings(UUID, TEXT, TIMESTAMPTZ, INT, INT) IS
  'Consensus rankings for a community (paginated). score = 0.7 * unique_listeners + 0.3 * sum(min(user plays, 3)).';
