...
usersRouter.get("/:username/followers", async (req, res) => {
  ...
  try {
    const result = await getFollowListWithStatus(
      user.id,
      viewerId,
      "followers",
      {
...
usersRouter.get("/:username", async (req, res) => {
  ...
  const [followersRes, followingRes, reviewsCountRes, streakRes] =
    await Promise.all([
      supabase
        .from("follows")
  ...
  if (followersRes.error || followingRes.error) {
    return internalError(
      res,
      `Failed to load counts: ${followersRes.error?.message} ${followingRes.error?.message}`,
    );
  }
