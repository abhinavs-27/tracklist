...
import {
  ...
  type ArtistReview,
  type PopularAlbumsForArtistResult,
  type ArtistPopularTrack,
} from "@/lib/queries";
...
function CommunityStatsLine({
  popularAlbumsPromise,
}: {
  popularAlbumsPromise: Promise<PopularAlbumsForArtistResult>;
}) {
  const popularAlbumsResult = use(popularAlbumsPromise);
  const popularAlbums = popularAlbumsResult.rows;
  const showAlbumsViewMore = popularAlbumsResult.hasMoreAlbums;

  const totalCommunityPlays = popularAlbums.reduce(
  ...
  const avgRating =
    ratedAlbums.length > 0
      ? ratedAlbums.reduce((s, a) => s + (a.average_rating ?? 0), 0) /
        ratedAlbums.length
      : null;
...
function ViewerArtistRelationship({
  viewerStatsPromise,
  firstListenPromise,
}: {
  viewerStatsPromise: Promise<{
    playCount: number;
    topAlbumName: string | null;
    topAlbumId: string | null;
  } | null>;
  firstListenPromise: Promise<string | null>;
}) {
  const viewerStats = use(viewerStatsPromise);
  const firstListened = use(firstListenPromise);

  if (!viewerStats || viewerStats.playCount === 0) return null;
...
function TopTracksSection({
  ...
  promise: Promise<ArtistPopularTrack[]>;
}) {
  const topTracks = use(promise);
  if (!topTracks.length) return null;
  return <ArtistPopularTracks tracks={topTracks} />;
}

function PopularAlbumsSection({
  promise,
  artistName,
  entityId,
}: {
  promise: Promise<PopularAlbumsForArtistResult>;
  artistName: string;
  entityId: string;
}) {
  const popularAlbumsResult = use(promise);
  const popularAlbums = popularAlbumsResult.rows;
  const showAlbumsViewMore = popularAlbumsResult.hasMoreAlbums;

  if (popularAlbums.length === 0) return null;
...
function RecentReviewsSection({
  ...
  promise: Promise<ArtistReview[]>;
}) {
  const recentReviews = use(promise);
              ...
              <Link href={entityHref} className="group flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-white group-hover:text-emerald-400 group-hover:underline">
                    {r.entity_name ??
                      (r.entity_type === "album" ? "Album" : "Track")}
                  </p>
                  <p className="mt-0.5 text-base text-amber-400 leading-none">
                    {formatStarDisplay(
                      Math.max(0, Math.min(5, Number(r.rating))),
                    )}
                  </p>
                </div>
              </Link>
...
export async function ArtistPageContent({ params }: { params: PageParams }) {
  ...
  const topTracksPromise = withArtistPagePhaseLog(
    "getTopTracksForArtist",
    id,
    getTopTracksForArtist(id, 10),
  );
  const reviewsPromise = withArtistPagePhaseLog(
    "getReviewsForArtist",
    id,
    getReviewsForArtist(id, 6),
  );
  const popularAlbumsPromise = withArtistPagePhaseLog(
    "getPopularAlbumsForArtist",
    id,
    getPopularAlbumsForArtist(id, 8),
  );

  const [session, artistFetched] = await Promise.all([
    sessionPromise,
    artistFetchedPromise,
  ]);
  ...
  const viewerStatsPromise = viewerId
    ? getViewerArtistStats(viewerId, entityId).catch(() => null)
    : Promise.resolve(null);
  const firstListenPromise = viewerId
  ...
  return (
    ...
    <div className="space-y-8">
      ...
      {/* Community stats */}
      <Suspense fallback={<div className="h-5 w-48 animate-pulse rounded bg-zinc-800/40" />}>
        <CommunityStatsLine popularAlbumsPromise={popularAlbumsPromise} />
      </Suspense>

      {/* Your relationship with this artist */}
      <Suspense fallback={<div className="h-11 w-full animate-pulse rounded-2xl bg-zinc-900/40" />}>
        <ViewerArtistRelationship
          viewerStatsPromise={viewerStatsPromise}
          firstListenPromise={firstListenPromise}
        />
      </Suspense>
      ...
      <ArtistTabs
        ...
        generalContent={
          <div className="space-y-8">
            <Suspense fallback={<div className="h-64 w-full animate-pulse rounded-2xl bg-zinc-900/40" />}>
              <TopTracksSection promise={topTracksPromise} />
            </Suspense>

            <Suspense fallback={<div className="h-96 w-full animate-pulse rounded-2xl bg-zinc-900/40" />}>
              <PopularAlbumsSection
                promise={popularAlbumsPromise}
            ...
            <Suspense fallback={<div className="h-64 w-full animate-pulse rounded-2xl bg-zinc-900/40" />}>
              <RecentReviewsSection promise={reviewsPromise} />
            </Suspense>
          </div>
        }
        socialContent={
