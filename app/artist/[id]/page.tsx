...
async function getLogsForSpotify(spotifyId: string): Promise<LogWithUser[]> {
  ...
  const res = await fetch(`${base}/api/logs?spotify_id=${encodeURIComponent(spotifyId)}&limit=20`, {
    cache: 'no-store',
  });
  if (!res.ok) return [];
  return res.json();
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-end">
        ...
        <div className="min-w-0">
          ...
          {artist.genres?.length ? (
            <p className="mt-1 text-zinc-400">{artist.genres.slice(0, 5).join(' · ')}</p>
          ) : null}
      ...
      {logs.length > 0 && (
        <section>
          ...
          <p className="mb-2 text-sm text-zinc-500">Logs for this artist (albums/tracks).</p>
