    ...
    <div className="space-y-10">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className={i >= 2 ? "max-md:hidden" : undefined}>
              <SkeletonCard imageHeight={100} lines={1} />
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className={`flex items-center gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3 ${i >= 4 ? "max-md:hidden" : ""}`}
            >
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className={i >= 4 ? "max-md:hidden" : undefined}>
              <SkeletonCard imageHeight={80} lines={2} />
            </div>
          ))}
        </div>
      </section>
