...
function comparisonLine(logsDiffPercent: number | null): string {
  if (logsDiffPercent == null) return "No prior week to compare — keep logging.";
...
export default async function WeeklyReportPage({
  ...
  if (period === "week") {
    ...
    return <WeeklyStoryPage story={story} offset={offset} basePath="/reports/week" />;
  }
  ...
  if (report.top_artist_id) {
    ...
    } catch { topArtist = null; }
  }
  if (report.top_album_id) {
    try {
      const data = await getOrFetchAlbum(report.top_album_id, { allowNetwork: true });
  ...
  if (report.top_track_id) {
    try {
      const { track } = await getOrFetchTrack(report.top_track_id, { allowNetwork: true });
  ...
  return (
    <div className="space-y-6">
      <PeriodSelector basePath={base} period={period} />
      <div className="flex flex-wrap items-center gap-3">
        <p className="text-zinc-400">{report.period_label}</p>
        <nav className="flex gap-2" aria-label="Previous or next period">
          <Link href={prevUrl} className="rounded-lg border border-zinc-600 bg-zinc-800/50 px-3 py-1.5 text-sm text-zinc-300 hover:bg-zinc-700">
            ← Previous
          </Link>
          {nextUrl && (
            <Link href={nextUrl} className="rounded-lg border border-zinc-600 bg-zinc-800/50 px-3 py-1.5 text-sm text-zinc-300 hover:bg-zinc-700">
              Next →
            </Link>
          )}
        </nav>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatBox label="Total listens" value={String(report.listen_count)} />
        <TopBox label="Top artist" entity={topArtist} hrefPrefix="/artist" rounded="full" />
        <TopBox label="Top album" entity={topAlbum} hrefPrefix="/album" rounded="md" />
        <TopBox label="Top track" entity={topTrack} hrefPrefix="/song" rounded="md" />
      </div>
    </div>
  );
}
...
function PeriodSelector({ basePath, period }: { basePath: string; period: PeriodType }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Link href={`${basePath}?period=week&offset=0`} className={btn("week", "Week")}>Week</Link>
      <Link href={`${basePath}?period=month&offset=0`} className={btn("month", "Month")}>Month</Link>
      <Link href={`${basePath}?period=year&offset=0`} className={btn("year", "Year")}>Year</Link>
    </div>
  );
}
...
function TopBox({ label, entity, hrefPrefix, rounded }: { label: string; entity: { name: string; id: string; imageUrl: string | null } | null; hrefPrefix: string; rounded: "full" | "md" }) {
  const imgClass = `h-12 w-12 shrink-0 overflow-hidden bg-zinc-800 ${rounded === "full" ? "rounded-full" : "rounded"}`;
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
      <p className="text-xs font-medium uppercase tracking-wide text-zinc-500">{label}</p>
      <div className="mt-2 flex items-center gap-3">
        {entity ? (
          <>
            <Link href={`${hrefPrefix}/${entity.id}`} className={imgClass}>
              {entity.imageUrl ? (
...
function WeeklyStoryPage({
  ...
  return (
    <div className="space-y-6">
      <PeriodSelector basePath={basePath} period="week" />
      ...
      <header className="space-y-2">
        ...
        <p className="text-xl font-medium leading-snug text-white sm:text-2xl">{summary}</p>
      ...
      <section aria-label="Top artist, album, and track">
        <h2 className="mb-3 text-sm font-medium uppercase tracking-wide text-zinc-500">Top picks</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          <TopCard label="Artist" entity={top.artist} hrefPrefix="/artist" />
      ...
      {insights.length > 0 && (
        <section aria-label="Insights">
          <h2 className="mb-3 text-sm font-medium uppercase tracking-wide text-zinc-500">Insights</h2>
      ...
      <section className="rounded-lg border border-zinc-800/80 bg-zinc-900/40 px-4 py-3" aria-label="Week over week">
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
      ...
      <div className="mt-3 flex items-center gap-3">
        {entity?.id ? (
          <>
            <Link href={`${hrefPrefix}/${entity.id}`} className="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-zinc-800 sm:h-16 sm:w-16">
              {entity.imageUrl ? (
                <img src={entity.imageUrl} alt="" className="h-full w-full object-cover" />
              ) : (
                <div className="flex h-full w-full items-center justify-center text-lg text-zinc-500">♪</div>
