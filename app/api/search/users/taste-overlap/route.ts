...
export const GET = withHandler(
  async (request: NextRequest, { userId: meId }) => {
    ...
    const rows = await getTasteOverlapSuggestionsForViewer(meId!, { limit });
    const withoutScore = rows.map(({ score: _s, ...u }) => u);
    const users = await enrichUsersWithFollowStatus(withoutScore, meId!);
