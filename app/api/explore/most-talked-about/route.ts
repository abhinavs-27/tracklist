...
export const GET = withHandler(async (request) => {
  ...
  exploreLogLine("explore/most-talked-about: start");
  const bypassCache = request.nextUrl.searchParams.get("refresh") === "1";
  const range = parseRange(request.nextUrl.searchParams.get("range"));

  try {
    const res = await staleFirstApiOk(
      `explore:most-talked:${exploreRangeToRpc(range)}:v1`,
      STALE_FIRST_TTL_SEC.exploreDiscoverySection,
      STALE_FIRST_STALE_AFTER_SEC.exploreDiscoverySection,
      async () => ({ items: await getExploreMostTalkedAbout(range, 14) }),
      { bypassCache },
    );
    exploreLogLine(`explore/most-talked-about: done: ${Date.now() - start} ms`);
    return res;
  } catch (e) {
