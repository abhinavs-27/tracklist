...
import {
  resolveAndCheckPending,
  ResolveEntityOutcome,
...
function handlePending(res: Extract<ResolveEntityOutcome, { kind: 'pending' }>) {
  return apiServiceUnavailable(
    `Catalog is syncing this ${res.entity} from Spotify. Retry in a few seconds.`,
    {
...
export const POST = withHandler(
  async (request, { user: me }) => {
    ...
    if (artistRes?.kind === 'pending') {
      return handlePending(artistRes);
    }
