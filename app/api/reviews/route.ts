...
export const POST = withHandler(
  async (request, { user: me }) => {
    ...
    /**
     * Optimization: parallelize side effects (achievements, feed events, community fan-out)
     * and user summary fetching to reduce total response latency.
     */
    const [userRow] = await Promise.all([
      ...
      (async () => {
        try {
          ...
          console.warn("[reviews] grantAchievementOnReview failed", e);
        }
      })(),
      (async () => {
        try {
          ...
          console.warn("[reviews] recordRatingFeedEvent failed", e);
        }
      })(),
      (async () => {
        try {
          ...
          console.warn("[reviews] community_feed fan-out failed", e);
        }
      })(),
    ]);
