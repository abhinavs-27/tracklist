  ...
  return (
    <div className={sectionGap}>
      ...
      <Suspense
        fallback={
          <section>
            <div className="mb-3 h-7 w-48 animate-pulse rounded-lg bg-zinc-800/60" />
            <div className="h-4 w-64 animate-pulse rounded bg-zinc-800/40" />
            <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="aspect-square animate-pulse rounded-xl bg-zinc-900/50"
                />
              ))}
            </div>
          </section>
      ...
      >
        <FansAlsoLikeSection songId={id} songName={track.name} />
      </Suspense>
