...
export default async function SongPage({ params }: { params: PageParams }) {
  ...
  const [reviewsDataRes, stats, recentListens, relatedSongsRaw] = await Promise.all([
    ...
    getListenLogsForTrack(entityId, 10, 0, viewerId, supabase).catch((e) => {
  ...
  const reviewsData = reviewsDataRes!;
