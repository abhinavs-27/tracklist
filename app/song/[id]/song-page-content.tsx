...
export async function SongPageContent({
  id,
  entityId,
  track,
}: {
  ...
  const [reviewsRes, statsRes, recentListensRes, relatedSongsRes] = await Promise.allSettled([
  ...
  const entityStats =
    statsRes.status === "fulfilled"
      ? statsRes.value
      : { listen_count: 0, average_rating: null, review_count: 0 };

  const recentListens =
    recentListensRes.status === "fulfilled" ? recentListensRes.value : [];
  const relatedSongsRaw =
    relatedSongsRes.status === "fulfilled" ? relatedSongsRes.value : [];
    ...
    <div className={sectionGap}>
      ...
      <div className="flex flex-col items-center gap-8 sm:flex-row sm:items-end sm:gap-10">
        ...
        <div className="w-full min-w-0 flex-1 text-left">
          ...
          <SongStatsBar songId={entityId} serverStats={entityStats} />
