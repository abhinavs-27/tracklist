...
await esbuild.build({
  entryPoints: {
    handler: path.join(root, "infra/aws/lambda/enrich-drain-scheduler/entry.ts"),
  },
  bundle: true,
  platform: "node",
  target: "node20",
  outdir: path.join(root, "infra/aws/lambda/enrich-drain-scheduler/dist"),
  sourcemap: true,
  alias: {
    "@": root,
...
console.log("Built infra/aws/lambda/enrich-drain-scheduler/dist/handler.js");
console.log("EventBridge schedule: rate(1 hour)");
